...
from ..plugins import ALIVE_NAME, USERID, godfather_grp, mention
...
@godfather.tgbot.on(CallbackQuery(data=re.compile(b"help_k_minu")))
@check_owner
...
def main_menu():
    ...
    buttons = [
        ...
        [
            ...
            Button.url("Support 🇮🇳", "https://t.me/godfatherBot_AI"),
        ],
...
@godfather.tgbot.on(InlineQuery)
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**godfatherBot"):
            buttons = [
                (Button.url(f"{ALIVE_NAME}", f"tg://openmessage?user_id={USERID}"),),
                (
                    Button.inline("Stats", data="stats"),
                    Button.url("Repo", "https://github.com/ITS-godfatherBOT/godfatherBOT"),
                ),
            ]
        ...
        if query.startswith("**⚜ godfatherBot"):
            grp_username = gvarstatus("GROUP_USERNAME") or "godfatherBot_OP"
            chnl_username = gvarstatus("CHANNEL_USERNAME") or "godfatherBot_AI"
            buttons = [
                (Button.url(f"{ALIVE_NAME}", f"tg://openmessage?user_id={USERID}"),),
                (
            ...
            result = builder.article(
                title="Repository",
                text=f"**⚜ godfatherary Af godfatherBot ⚜**",
                buttons=[
                    [Button.url("♥️ Tutorial ♥", "https://youtu.be/CH_KO1wim2o")],
                    [Button.url("📍 𝚁𝚎𝚙𝚘 📍", "https://github.com/godfather-AI/godfatherBot")],
                    [
                        Button.url(
                            "💞 Deploy 💞",
                            "https://heroku.com/deploy?template=https://github.com/godfather-AI/godfatherBOT",
                        )
                    ],
                ],
            )
            ...
            buttons = []
            media = None
            godfathermedia = MEDIA_PATH_REGEX.search(markdown_note)
            if godfathermedia:
                media = godfathermedia.group(2)
                markdown_note = markdown_note.replace(godfathermedia.group(0), "")
            ...
            builder = event.builder
            troll = os.path.join("./godfatherbot", "troll.txt")
            try:
            ...
            try:
                ...
                try:
                    ...
                    if u.username:
                        godfather = f"@{u.username}"
                    else:
                        godfather = f"[{u.first_name}](tg://user?id={u.id})"
                    u = int(u.id)
                except ValueError:
                    # ValueError: Could not find the input entity
                    godfather = f"[user](tg://user?id={u})"
            except ValueError:
                # if u is username
                try:
            ...
            result = builder.article(
                title="Troll Message",
                text=f"🌹 Only This : {godfather} cannot access this message !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./godfatherbot", "secrets.txt")
            try:
            ...
            result = builder.article(
                ...
                text=f"🔒 A whisper message to {godfather}, Only he/she can open it.",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./godfatherbot", "hide.txt")
            try:
            ...
            if oso == "OFF":
                result = builder.article(
                    title="© godfatherBot Help",
                    description="Help menu for godfatherBot",
                    text=_result[0],
                    buttons=_result[1],
                    link_preview=False,
                )
            ...
            if PM_IMG == "OFF":
                ...
                godfather = [x for x in PM_IMG.split()]
                PIC = list(godfather)
                godfather_IMG = random.choice(PIC)
            query = gvarstatus("pmpermit_text")
            if godfather_IMG and godfather_IMG.endswith((".jpg", ".jpeg", ".png")):
                result = builder.photo(
                    godfather_IMG,
                    # title="Alive godfather",
                    text=query,
            ...
            buttons = [
                (
                    Button.url("Source code", "https://github.com/godfather-AI/godfatherBOT"),
                    Button.url(
                        "Deploy",
                        "https://dashboard.heroku.com/new?button-url=https%3A%2F%2Fgithub.com%2Fgodfather-AI%2FgodfatherBOT&template=https%3A%2F%2Fgithub.com%2Fgodfather-AI%2FgodfatherBOT",
                    ),
                )
            ]
            ...
            result = types.InputBotInlineResult(
                ...
                type="photo",
                title=f"Lêɠêɳ̃dẞø†",
                description=f"Lêɠêɳ̃dẞø†\nhttps://t.me/godfatherBot_OP",
                url="https://github.com/godfather-AI/godfatherBOT",
                thumb=photo,
                content=photo,
                send_message=types.InputBotInlineMessageMediaAuto(
                    reply_markup=markup, message=text, entities=msg_entities
...
@godfather.tgbot.on(CallbackQuery(data=re.compile(b"clise")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    buttons = [
    ...
    await event.edit(
        f"📜 Menu Provider Has Been Closed\n\n🔰 Bot Of : {mention}\n\n             [©️Lêɠêɳ̃dẞø†](https://t.me/godfatherBot_OP)",
        buttons=buttons,
        link_preview=False,
    )


@godfather.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@godfather.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@godfather.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
