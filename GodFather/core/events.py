...
async def safe_check_text(msg):  # sourcery no-metrics
    ...
    phone = str((await godfather.get_entity(godfather.uid)).phone)
    return bool(
        (
            (Config.GODFATHER_STRING in msg)
            or (phone[-10:] in msg)
...
async def send_message(
    ...
    *,
    ...
    if safecheck:
        ...
        msg = f"**Sorry, This Is Sensitive Data I Cant Send It To Public.& Reported to Admin Of godfatherBot Group [admin](https://t.me/godfatherBot_OP). & Dont Try To Send Any Information Without Knowing Anything.** ▶️ [Logger group]({msglink})"
        return await client.sendmessage(
            entity=chatid,
            message=msg,
            send_as=send_as,
