...
from GodFather import Config, godfather
...
@godfather.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"rules")))
...
@godfather.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"users")))
...
@godfather.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"depy")))
async def help(event):
    if event.query.user_id == bot.uid:
        await event.answer("This Is Not For U My Master", cache_time=0, alert=True)
    else:
        await event.client.send_message(
            event.chat_id,
            message="You Can Deploy/Install GodFather In Heroku By Following Steps Bellow, You Can See Some Quick Guides On Support Channel Or On Your Own Assistant Bot. \nThank You For Contacting Me.",
            link_preview=False,
            buttons=[
                [
                ...
                [
                    Button.url(
                        "Github Repo ❓", "https://github.com/godfather-AI/GodFather"
                    ),
                ],
            ],
        )
...
@godfather.bot_cmd(incoming=True, func=lambda e: e.is_private)
...
@godfather.bot_cmd(pattern="^/uinfo$", from_users=Config.OWNER_ID)
async def bot_start(event):
    reply_to = await reply_id(event)
    if not reply_to:
    ...
    if found:
        if flood_count >= FloodConfig.AUTOBAN:
            if user_.id in Config.SUDO_USERS:
                ...
                if BOTLOG:
                    await godfather.tgbot.send_message(BOTLOG_CHATID, sudo_spam)
            else:
        ...
        if not fa_id:
            return
        try:
            msg_ = await godfather.tgbot.get_messages(BOTLOG_CHATID, fa_id)
        ...
        if BOTLOG:
            fa_msg = await godfather.tgbot.send_message(
                BOTLOG_CHATID,
                flood_msg,
                buttons=buttons,
            )
        try:
            chat = await godfather.tgbot.get_entity(BOTLOG_CHATID)
            await godfather.tgbot.send_message(
                Config.OWNER_ID,
...
@godfather.tgbot.on(CallbackQuery(data=re.compile(b"bot_pm_ban_([0-9]+)")))
@check_owner
async def bot_pm_ban_cb(c_q: CallbackQuery):
    user_id = int(c_q.pattern_match.group(1))
    try:
        user = await godfather.get_entity(user_id)
    except Exception as e:
...
@godfather.tgbot.on(CallbackQuery(data=re.compile(b"toggle_bot-antiflood_off$")))
@check_owner
...
@godfather.bot_cmd(incoming=True, func=lambda e: e.is_private)
@godfather.bot_cmd(edited=True, func=lambda e: e.is_private)
