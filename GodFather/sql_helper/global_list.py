...
class godfatherGloballist(BASE):
    __tablename__ = "godfatherglobal_list"
    ...
    def __repr__(self):
        return "<godfather global values '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, godfatherGloballist)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


godfatherGloballist.__table__.create(checkfirst=True)

godfatherGLOBALLIST_INSERTION_LOCK = threading.RLock()
...
def add_to_list(keywoard, group_id):
    with godfatherGLOBALLIST_INSERTION_LOCK:
        broadcast_group = godfatherGloballist(keywoard, str(group_id))
...
def rm_from_list(keywoard, group_id):
    with godfatherGLOBALLIST_INSERTION_LOCK:
        broadcast_group = SESSION.query(godfatherGloballist).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_list(keywoard):
    with godfatherGLOBALLIST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(godfatherGloballist.keywoard)
            .filter(godfatherGloballist.keywoard == keywoard)
            .delete()
        )
...
def get_list_keywords():
    try:
        chats = SESSION.query(godfatherGloballist.keywoard).distinct().all()
...
def num_list():
    try:
        return SESSION.query(godfatherGloballist).count()
    finally:
        SESSION.close()
...
def num_list_keywords():
    try:
        return SESSION.query(func.count(distinct(godfatherGloballist.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_lists():
    try:
        ...
        all_groups = SESSION.query(godfatherGloballist).all()
        for x in all_groups:
