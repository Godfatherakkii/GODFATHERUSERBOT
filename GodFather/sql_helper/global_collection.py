...
class godfather_GlobalCollection(BASE):
    __tablename__ = "godfather_globalcollection"
    ...
    def __repr__(self):
        return "<godfather Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, godfather_GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


godfather_GlobalCollection.__table__.create(checkfirst=True)

godfather_GLOBALCOLLECTION = threading.RLock()
...
def add_to_collectionlist(keywoard, contents):
    with godfather_GLOBALCOLLECTION:
        keyword_items = godfather_GlobalCollection(keywoard, tuple(contents))
...
def rm_from_collectionlist(keywoard, contents):
    with godfather_GLOBALCOLLECTION:
        if keyword_items := SESSION.query(godfather_GlobalCollection).get(
...
def del_keyword_collectionlist(keywoard):
    with godfather_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(godfather_GlobalCollection.keywoard)
            .filter(godfather_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(godfather_GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist():
    try:
        return SESSION.query(godfather_GlobalCollection).count()
    finally:
        SESSION.close()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(godfather_GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(godfather_GlobalCollection).all()
        for x in all_groups:
