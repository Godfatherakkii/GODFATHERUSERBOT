...
class Gdrive(BASE):
    __tablename__ = "godfathergdrive"
    godfather = Column(String(50), primary_key=True)
...
def is_folder(folder_id):
    try:
        return SESSION.query(Gdrive).filter(Gdrive.godfather == str(folder_id))
...
def rmparent_id(folder_id):
    if note := SESSION.query(Gdrive).filter(Gdrive.godfather == folder_id):
        note.delete()
        SESSION.commit()
