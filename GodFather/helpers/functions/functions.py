...
async def fileinfo(file):
    ...
    godfather_json = json.loads(x)["media"]["track"]
    dic = {
        "path": file,
        "size": int(godfather_json[0]["FileSize"]),
        "extension": godfather_json[0]["FileExtension"],
    }
    try:
        if "VideoCount" or "AudioCount" or "ImageCount" in cat_json[0]:
            dic["format"] = godfather_json[0]["Format"]
            dic["type"] = godfather_json[1]["@type"]
            if "ImageCount" not in godfather_json[0]:
                dic["duration"] = int(float(godfather_json[0]["Duration"]))
                dic["bitrate"] = int(int(godfather_json[0]["OverallBitRate"]) / 1000)
            if "VideoCount" or "ImageCount" in godfather_json[0]:
                dic["height"] = int(godfather_json[1]["Height"])
                dic["width"] = int(godfather_json[1]["Width"])
...
async def animator(media, mainevent, textevent):
    ...
    godfatherOp = await mainevent.client.download_media(media, Config.TEMP_DIR)
    await textevent.edit("__🎞Converting into Animated sticker..__")
    await runcmd(
        f"ffmpeg -ss 00:00:00 -to 00:00:02.900 -i {godfatherOp} -vf scale={w}:{h} -c:v libvpx-vp9 -crf 30 -b:v 560k -maxrate 560k -bufsize 256k -an animate.webm"
    )  # pain
    os.remove(godfatherOp)
...
async def clippy(borg, msg, chat_id, reply_to_id):
    ...
    async with borg.conversation(chat) as conv:
        try:
            ...
            await godfather(unblock("clippy"))
            msg = await conv.send_file(msg)
...
def higlighted_text(
    ...
    if font_name is None:
        font_name = "godfatherbot/helpers/styles/impact.ttf"
    font = ImageFont.truetype(font_name, font_size)
...
async def waifutxt(text, chat_id, reply_to_id, bot):
    ...
    godfather = await sticcers[0].click("me", hide_via=True)
    if godfather:
        await bot.send_file(int(chat_id), godfather, reply_to=reply_to_id)
        await godfather.delete()
