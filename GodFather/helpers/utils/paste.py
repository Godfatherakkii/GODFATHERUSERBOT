...
async def d_paste(message, extension=None):
    ...
    """
    siteurl = "http://godfatherbin.up.railway.app/documents"
    data = {"content": message}
    try:
        response = requests.post(url=siteurl, data=json.dumps(data), headers=headers)
    except Exception as e:
    ...
    if response.ok:
        response = response.json()
        purl = (
            f"http://godfatherbin.up.railway.app/{response['key']}.{extension}"
            if extension
            else f"http://godfatherbin.up.railway.app/{response['key']}"
        )
        return {
            "url": purl,
            "raw": f"http://godfatherbin.up.railway.app/raw/{response['key']}",
            "bin": "Dog",
        }
