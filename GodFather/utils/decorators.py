    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.HANDLER) == 2:
                godfatherreg = f"^{Config.HANDLER}"
                reg = Config.HANDLER[1]
            elif len(Config.HANDLER) == 1:
                godfatherreg = f"^\\{Config.HANDLER}"
                reg = Config.HANDLER
            args["pattern"] = re.compile(godfatherreg + pattern)
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.SUDO_HANDLER) == 2:
                godfatherreg = f"^{Config.SUDO_HANDLER}"
...
def register(**args):
    ...
    def decorator(func):
        if not disable_edited:
            godfather.add_event_handler(func, MessageEdited(**args))
        godfather.add_event_handler(func, NewMessage(**args))
        try:
