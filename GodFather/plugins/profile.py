...
async def _(event):
    ...
    godfatherevent = await eor(event, "`Downloading Profile Picture to my local ...`")
    ...
    try:
        ...
        if photo:
            await godfatherevent.edit("`now, Uploading to Telegram ...`")
            if photo.endswith((".mp4", ".MP4")):
                # https://t.me/tgbetachat/324694
                size = os.stat(photo).st_size
                if size > 2097152:
                    await godfatherevent.edit("`size must be less than 2 mb`")
                    os.remove(photo)
                    return
                lolpic = None
            ...
            try:
                ...
                await eor(godfatherevent, "`My profile picture was successfully changed`")
    try:
        os.remove(photo)
    except Exception as e:
        LOGS.info(str(e))
