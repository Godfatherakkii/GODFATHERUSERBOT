# telegraph utils for godfatherUserBot
import os
import random
import string
...
from . import godfather, mention, reply_id
...
async def ctg(event):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await godfather(unblock("chotamreaderbot"))
            msg_flag = await conv.send_message(urls[0])
        response = await conv.get_response()
        await event.client.send_read_acknowledge(conv.chat_id)
        if response.text.startswith(""):
            await eor(godfatherevent, "Am I Dumb Or Are U Dumb?")
        else:
...
async def _(event):
    ...
    if input_str in ["media", "m"]:
        ...
        ms = (end - start).seconds
        godfather = f"https://telegra.ph/{response['path']}"
        await godfatherevent.edit(
            f"**✓ Uploaded to :-** [telegraph]({godfather})\
                 \n**✓ Uploaded in {ms} seconds.**\
                 \n**✓ Uploaded by :-** {mention}\
                 \n**✓ Telegraph :-** `{godfather}`",
            link_preview=True,
