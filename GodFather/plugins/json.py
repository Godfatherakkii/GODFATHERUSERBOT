...
async def _(event):
    ...
    godfatherevent = await event.get_reply_message() if event.reply_to_msg_id else event
    the_real_message = godfatherevent.stringify()
...
async def _(event):
    ...
    godfatherevent = await event.get_reply_message() if event.reply_to_msg_id else event
    the_real_message = _format.yaml_format(godfatherevent)
    await eor(event, the_real_message, parse_mode=_format.parse_pre)
