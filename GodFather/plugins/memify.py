# Made by @godfather_K_BOY and @godfather_K_BOY
# memify plugin for godfatherUserBot
import asyncio
import base64
import io
import os
...
from GodFather import BOTLOG_CHATID, godfather
...
async def maccmd(event):  # sourcery no-metrics
    ...
    godfatherevent = await event.edit("__Adding frame for media....__")
...
async def memes(event):
    ...
    if not godfatherinput:
        return await eod(
            event, "`what should i write on that u idiot give text to memify`"
        )
    if ";" in godfatherinput:
        top, bottom = godfatherinput.split(";", 1)
    else:
        top = godfatherinput
        bottom = ""
    ...
    meme = os.path.join("./temp", "godfathermeme.jpg")
    if gvarstatus("CNG_FONTS") is None:
        CNG_FONTS = "GodFather/helpers/styles/impact.ttf"
    else:
...
async def lang(event):
    ...
    if input_str not in font_list:
        ...
        await godfatherevent.edit(f"**Available Fonts names are here:-**\n\n{FONTS}")
    else:
        arg = f"GodFather/helpers/styles/{input_str}"
...
async def memes(event):
    ...
    color1 = c_list[0]
    color2 = c_list[1]
    bgcolor = "#080808" if not godfatherinput else godfatherinput
...
async def memes(event):
    ...
    godfatherinput = event.pattern_match.group(1)
    godfatherinput = 50 if not godfatherinput else int(godfatherinput)
    reply = await event.get_reply_message()
    if not reply:
        return await eod(event, "`Reply to supported Media...`")
    san = base64.b64decode("MFdZS2llTVloTjAzWVdNeA==")
    ...
    try:
        await crop(meme_file, outputfile, godfatherinput)
    except Exception as e:
...
async def memes(event):
    ...
    if not godfatherinput:
        godfatherinput = "50"
    if ";" in str(godfatherinput):
        godfatherinput, colr = godfatherinput.split(";", 1)
    else:
        colr = 0
    ...
    try:
        await add_frame(meme_file, outputfile, godfatherinput, colr)
    except Exception as e:
        return await output[0].edit(f"`{e}`")
    try:
        await event.client.send_file(
