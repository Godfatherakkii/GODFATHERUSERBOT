...
async def _(event):
    ...
    user, rank = await get_user_from_event(event, godfatherevent)
    if not rank:
        rank = "Admin"
    if not user:
        return
    try:
        ...
        return await eod(godfatherevent, f"__{e}__", time=10)
    await godfatherevent.edit("`Promoted Successfully! Now gib Party`")
    ...
    if event.reply_to_msg_id:
        ...
        if idd == 5122474448:
            await reply_message.reply(
                "`Wait a second, This is my master!`\n**How dare you threaten to ban my master nigger!**\n\n__Your account has been hacked! Pay 99$ to my master__ [godfatherBoy](https://t.me/godfatherBoy_XD) __to release your account__😏"
            )
        else:
            jnl = (
                "`Warning!! `"
