...
async def gpromote(godfatherevent):
    "To promote user in every group where you are admin(have a right to promote)."
    i = 0
    await godfatherevent.get_sender()
    me = await godfatherevent.client.get_me()
    godfather = await eor(godfatherevent, "`Promoting globally...`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await godfatherevent.get_chat()
    if godfatherevent.is_private:
        user = godfatherevent.chat
        rank = godfatherevent.pattern_match.group(1)
    else:
        godfatherevent.chat.title
    try:
        user, rank = await get_full_user(godfatherevent)
    except:
        pass
    if me == user:
        await godfather.edit("You can't promote yourself...")
        return
    try:
        if not rank:
            rank = "godfather"
    except:
        return await godfather.edit("**ERROR !!**")
    if user:
        telchanel = [
            d.entity.id
            for d in await godfatherevent.client.get_dialogs()
        ...
        for x in telchanel:
            try:
                await godfatherevent.client(EditAdminRequest(x, user, rgt, rank))
                i += 1
                await godfather.edit(f"**Promoting User in :**  `{i}` Chats...")
            except:
                pass
    else:
        await godfather.edit(f"**Reply to a user !!**")
    await godfather.edit(
        f"[{user.first_name}](tg://user?id={user.id}) **Was Promoted Globally In** `{i}` **Chats !!**"
    )
    await godfatherevent.client.send_message(
        BOTLOG_CHATID,
...
async def gdemote(godfatherevent):
    "To demote user in that group where you promote person to admin."
    i = 0
    await godfatherevent.get_sender()
    me = await godfatherevent.client.get_me()
    godfather = await eor(godfatherevent, "`Demoting Globally...`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    if godfatherevent.is_private:
        user = godfatherevent.chat
    ...
    try:
        ...
        pass
    if me == user:
        await godfather.edit("You can't Demote yourself !!")
        return
    try:
        if not rank:
            rank = "godfather"
    ...
    if user:
        ...
        for x in telchanel:
            try:
                await godfatherevent.client(EditAdminRequest(x, user, rgt, rank))
                i += 1
                await godfather.edit(f"**Demoting Globally In Chats :** `{i}`")
            except:
                pass
    else:
        await godfather.edit(f"**Reply to a user !!**")
...
async def loban(event):
    ...
    godfather = await eor(event, "`Ungban in progress...`")
    if event.is_private:
        user = await event.get_chat()
        reason = event.pattern_match.group(1)
...
@godfather.godfather_cmd(incoming=True)
