...
@godfather.godfather_cmd(
    ...
    info={
        ...
        "examples": ["{tr}wall godfather", "{tr}wall godfather ; 2"],
    },
)
async def noods(event):
    ...
    godfatherevent = await eor(event, "🔍 `Searching...`")
    r = requests.get(
    ...
    if not walls:
        return await eod(godfatherevent, f"**Can't find anything with** `{query}`", 10)
    i = count = 0
    piclist = []
    piclinks = []
    captionlist = []
    ...
    for x in walls:
        ...
        if "We are sorry," not in requests.get(a).text and a not in piclinks:
            await eor(godfatherevent, "📥** Downloading...**")
            pic = await wall_download(a, query)
            if pic is None:
                return await eod(godfatherevent, "__Sorry i can't download wallpaper.__")
            piclist.append(pic)
            piclinks.append(a)
            captionlist.append("")
            count += 1
            ...
            i += 1
        await eor(
            godfatherevent, f"**📥 Downloaded : {count}/{limit}\n\n❌ Errors : {i}/5**"
        )
        if count == int(limit):
            break
        if i == 5:
            await eor(godfatherevent, "`Max search error limit exceed..`")
    try:
        await eor(godfatherevent, "`Sending...`")
