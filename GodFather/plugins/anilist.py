...
async def anilist_usersearch(event):
    ...
    godfatherevent = await eor(event, "`Searching user profile in anilist...`")
    searchresult = await anilist_user(search_query)
    if len(searchresult) == 1:
        return await eor(
            godfatherevent, f"**Error while searching user profile:**\n{searchresult[0]}"
        )
...
async def get_manga(event):
    ...
    godfatherevent = await eor(event, "`Searching Manga..`")
...
async def get_anime(event):
    ...
    godfatherevent = await eor(event, "`Searching Anime..`")
    jikan = jikanpy.jikan.Jikan()
...
async def character(event):
    ...
    godfatherevent = await eor(event, "`Searching Character...`")
    try:
        search_result = jikan.search("character", search_query)
    except APIException:
        return await eod(godfatherevent, "`Character not found.`")
    first_mal_id = search_result["results"][0]["mal_id"]
...
async def anime_downld(event):  # sourcery no-metrics
    ...
    if input_str == "kaizoku":
        ...
        if search_result:
            ...
            result = f"<b>No result found for</b> <code>{html.escape(search_query)}</code> <b>on</b> <code>IndiAnime</code>"
    await godfatherevent.edit(result, parse_mode="html")
...
async def whatanime(event):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            f"https://api.trace.moe/search?anilistInfo&url={quote_plus(godfather)}"
        ) as raw_resp0:
