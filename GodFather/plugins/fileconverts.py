...
from ..helpers.utils import _format, _godfathertools, _godfatherutils, parse_pre, reply_id
...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    if mediatype in ["Gif", "Video"]:
        return await eod(event, "Reply to supported Media...")
    godfatherevent = await eor(event, "__Making round spin video wait a sec.....__")
    output = await _godfathertools.media_to_pic(event, reply, noedits=True)
    if output[1] is None:
        return await eod(
    ...
    if output is None:
        return await eod(godfatherevent, "__Unable to make spin gif.__")
    ...
    godfather = await event.client.send_file(
        event.chat_id,
        media,
        reply_to=reply,
        video_note=True,
...
async def video_swtfile(event):  # sourcery no-metrics
    ...
    type = True
    godfatherevent = await eor(event, "`Converting to round format..........`")
    ...
    if mediatype in ["Gif", "Video", "Sticker"]:
        if not swtfile.endswith((".webp")):
            if swtfile.endswith((".tgs")):
                hmm = await make_gif(godfatherevent, swtfile)
        ...
        if swtthumb is not None and os.path.exists(swtthumb):
            ...
            os.remove(swtfile)
            return await eod(godfatherevent, "`No thumb found to make it video note`", 5)
    if mediatype in [
        "Voice",
        "Audio",
        "Gif",
        ...
        if os.path.exists(PATH):
            ...
            uploaded = await event.client.fast_upload_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, godfatherevent, c_time, "Uploading....")
                ),
            )
            ul.close()
            media = types.InputMediaUploadedDocument(
...
async def _(godfather):
    reply_to_id = godfather.message.id
    if godfather.reply_to_msg_id:
        reply_to_id = godfather.reply_to_msg_id
    event = await eor(godfather, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await godfather.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await godfather.client.send_file(
                godfather.chat_id,
...
async def png(args):
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            ...
            await args.edit(f"`{random.choice(godfather)}`")
...
async def _(event):
    ...
    output = await _godfathertools.media_to_pic(event, reply)
    if output[1] is None:
        return await eod(
            output[0], "__Unable to extract image from the replied message.__"
        )
...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    godfatherevent = await eor(event, "__🎞 Making Gif from the relied media...__")
    imag = await _godfathertools.media_to_pic(event, reply, noedits=True)
    if imag[1] is None:
        return await eod(
    ...
    try:
        ...
        return await eod(godfatherevent, f"**Error**\n__{e}__")
    output = io.BytesIO()
    output.name = "Output.gif"
    ...
    if output is None:
        ...
        for i in [final, "Output.gif", imag[1]]:
            if os.path.exists(i):
                os.remove(i)
        return
    godfather = await event.client.send_file(event.chat_id, output, reply_to=reply)
    await _godfatherutils.unsavegif(event, godfather)
    await godfatherevent.delete()
    for i in [final, "Output.gif", imag[1]]:
        if os.path.exists(i):
...
async def _(event):
    ...
    if not args:
        ...
        try:
            ...
            args = 2.0
    godfatherevent = await eor(event, "__🎞Converting into Gif..__")
    ...
    godfather = await event.client.send_file(event.chat_id, result, reply_to=reply)
    await _godfatherutils.unsavegif(event, godfather)
    await godfatherevent.delete()
