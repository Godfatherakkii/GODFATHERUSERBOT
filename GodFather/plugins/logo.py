...
async def very(event):
    ...
    LOGO_BACKGROUND = (
        ...
        or "https://raw.githubusercontent.com/ITS-godfatherBOT/RESOURCES/master/background/black.jpg"
    )
    LOGO_FONT = (
        gvarstatus("LOGO_FONT")
        or "https://github.com/ITS-godfatherBOT/RESOURCES/blob/master/fonts/VampireWars.ttf?raw=true"
    )
...
async def lg1(event):
    ...
    LOGO_FONT_STROKE_WIDTH = gvarstatus("LOGO_FONT_STROKE_WIDTH") or 0
    LOGO_FONT_STROKE_COLOR = gvarstatus("LOGO_FONT_STROKE_COLOR") or None
    fnt = await get_font_file(event.client, "@godfather_Fonts")
    """
    cmd = event.pattern_match.group(1).lower()
    text = event.pattern_match.group(2)
    reply = await event.get_reply_message()
    ...
    if event.reply_to_msg_id:
        ...
        async for i in event.client.iter_messages(
            "@godfatherBot_Logos", filter=InputMessagesFilterPhotos
        ):
            PICS_STR.append(i)
...
async def bad(event):
    ...
    source = requests.get(
        "https://github.com/ITS-godfatherBOT/RESOURCES/tree/master/background"
    )
    ...
    if cmd == "c":
        ...
        if not input_str and event.reply_to_msg_id and reply_message.media:
            if not os.path.isdir("./temp"):
                os.mkdir("./temp")
            output = await _godfathertools.media_to_pic(event, reply_message)
    ...
    if input_str not in bg_name:
        godfatherevent = await eor(event, "`Give me a correct background name...`")
        await asyncio.sleep(1)
        await eod(godfatherevent, lbg_list, time=60)
    else:
        string = f"https://raw.githubusercontent.com/ITS-godfatherBOT/RESOURCES/master/background/{input_str}.jpg"
...
async def pussy(event):
    ...
    if cmd == "":
        source = requests.get(
            "https://github.com/ITS-godfatherBOT/RESOURCES/tree/master/fonts"
        )
        soup = BeautifulSoup(source.text, features="html.parser")
        links = soup.find_all("a", class_="js-navigation-open Link--primary")
        logo_font = []
        ...
        if input_str not in logo_font:
            godfatherevent = await eor(event, "`Give me a correct font name...`")
            await asyncio.sleep(1)
            await eod(godfatherevent, font_name, time=80)
        else:
            if " " in input_str:
                input_str = str(input_str).replace(" ", "%20")
            string = f"https://github.com/ITS-godfatherBOT/RESOURCES/blob/master/fonts/{input_str}.ttf?raw=true"
        ...
        if input_str not in fg_name:
            godfatherevent = await eor(event, "`Give me a correct color name...`")
            await asyncio.sleep(1)
            await eod(
                godfatherevent,
                f"**Available color names are here:-**\n\n{fg_list}",
        ...
        godfather = re.compile(r"^\-?[1-9][0-9]*\.?[0-9]*")
        isint = re.match(godfather, input_str)
