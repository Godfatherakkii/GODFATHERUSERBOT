...
async def grpinfo():
    outstr = "**Plugins in godfatherBot are:**\n\n"
    outstr += f"**👨‍💻 Usage : ** `{cmdprefix}help <plugin name>`\n\n"
    category = ["admin", "bot", "fun", "misc", "tools", "utils", "extra", "useless"]
    for godfather in category:
        plugins = GRP_INFO[godfather]
        outstr += f"**{hemojis[godfather]} {godfather.title()} **({len(plugins)})\n"
...
async def cmdlist():
    outstr = "**Total list of Commands in your godfatherBot are :**\n\n"
...
@godfather.godfather_cmd(
    ...
    info={
        "header": "To get guide for godfatherBot.",
...
async def _(event):
    ...
    if not input_str:
        ...
        for cmd in cmds:
            outstr += f"  - `{cmdprefix}{cmd}`\n"
        outstr += f"**👨‍💻  Usage : ** `{cmdprefix}help -l <command name>`"
    await eor(event, outstr, aslink=True, linktext="Total Commands of godfatherBot are :")
