...
# godfatheruserbot Google Drive managers  ported from Projectbish and added extra things by @godfather_K_BOY
...
async def download(event, gdrive, service, uri=None):  # sourcery no-metrics
    ...
    if uri:
        ...
        if swttorrent:
            ...
            await eor(
                gdrive,
                "`To use torrent files or download files from link install torrentutils from` @godfatherplugins",
            )
...
async def upload(gdrive, service, file_path, file_name, mimeType, dir_id=None):
    ...
    body = {
        "name": file_name,
        "description": "Uploaded from Telegram using godfatheruserbot.",
        "mimeType": mimeType,
...
async def g_download(event):
    ...
    godfatherevent = await eor(event, "`Downloading Requested File from G-Drive...`")
    file_name, swtprocess = await gdrive_download(
        event, godfatherevent, service, drive_link
    )
    if swtprocess is not None:
        return await eod(godfatherevent, file_name)
    ...
    if not cmd:
        ...
        await event.client.send_file(
            ...
            progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                progress(d, t, godfatherevent, c_time, "Uploading...", file_name)
            ),
        )
        os.remove(file_name)
        await eod(
...
async def gshare(event):
    ...
    godfatherevent = await eor(event, "`Creating sharable link...`")
    await asyncio.sleep(2)
    await share(service, godfatherevent, input_str)
