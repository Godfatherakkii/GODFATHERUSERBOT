...
autopic_path = os.path.join(os.getcwd(), "GodFather", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "GodFather", "digital_pic.png")
autophoto_path = os.path.join(os.getcwd(), "GodFather", "photo_pfp.png")
...
async def autopicloop():
    ...
    if AUTOPICSTART and DEFAULT_PIC is None:
        if BOTLOG:
            return await godfather.send_message(
                BOTLOG_CHATID,
    ...
    while AUTOPICSTART:
        ...
        file = await godfather.upload_file(autophoto_path)
        try:
            await godfather(functions.photos.UploadProfilePhotoRequest(file))
...
async def custompfploop():
    ...
    while CUSTOMPICSTART:
        ...
        file = await godfather.upload_file("donottouch.jpg")
        try:
            if i > 0:
                await godfather(
                    functions.photos.DeletePhotosRequest(
                        await godfather.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await godfather(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        LOGS.info(name)
        try:
            await godfather(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await godfather(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(CHANGE_TIME)
...
async def autopfp_start():
    ...
    while AUTOPFP_START:
        ...
        i += 1
        await godfather(functions.photos.UploadProfilePhotoRequest(file))
        await _godfatherutils.runcmd("rm -rf donottouch.jpg")
        await asyncio.sleep(CHANGE_TIME)
...
godfather.loop.create_task(autopfp_start())
godfather.loop.create_task(autopicloop())
godfather.loop.create_task(digitalpicloop())
godfather.loop.create_task(bloom_pfploop())
godfather.loop.create_task(autoname_loop())
godfather.loop.create_task(autobio_loop())
godfather.loop.create_task(custompfploop())
