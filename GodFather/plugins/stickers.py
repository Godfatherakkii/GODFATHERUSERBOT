...
# modified and developed by @godfather_K_BOY
...
def pack_name(userid, pack, is_anim, is_vid):
    if gvarstatus("CUSTOM_STICKER_SETNAME") is not None:
        ...
        return f"GodFather_{userid}_{pack}_anim"
    elif is_vid:
        return f"GodFather_{userid}_{pack}_vid"
    else:
        return f"GodFather_{userid}_{pack}"
...
async def delpack(godfatherevent, conv, args, packname):
    try:
        await conv.send_message("/delpack")
    except YouBlockedUserError:
        await godfather(unblock("stickers"))
        await conv.send_message("/delpack")
...
async def add_to_pack(
    ...
    while ("50" in x.message) or ("120" in x.message) or vtry:
        ...
        await godfatherevent.edit(f"`Switching to Pack {pack} due to insufficient space`")
        await conv.send_message(packname)
        x = await conv.get_response()
...
async def kang(args):  # sourcery no-metrics
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            godfatherevent = await eor(args, f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
            ...
            if message.media.document.mime_type == "video/webm":
                ...
                for attribute in attributes:
                    if isinstance(attribute, DocumentAttributeSticker):
                        if message.media.document.size / 1024 > 255:
                            godfatherevent = await eor(
                                args, "__⌛ File size big,,, Downloading..__"
                            )
                            sticker = await animator(message, args, godfatherevent)
                            await eor(godfatherevent, f"`{random.choice(KANGING_STR)}`")
                        else:
                            godfatherevent = await eor(
                ...
                godfatherevent = await eor(args, "__⌛ Downloading..__")
                sticker = await animator(message, args, godfatherevent)
                await eor(godfatherevent, f"`{random.choice(KANGING_STR)}`")
        else:
    ...
    if photo:
        ...
        if len(splat) == 2:
            if char_is_emoji(splat[0][0]):
                if char_is_emoji(splat[1][0]):
                    return await godfatherevent.edit("check `.info stickers`")
        ...
        if (
            ...
            await godfatherevent.edit("`Brewing a new Pack...`")
            async with args.client.conversation("@Stickers") as conv:
...
async def pack_kang(args):  # sourcery no-metrics
    ...
    godfather = base64.b64decode("MFdZS2llTVloTjAzWVdNeA==")
    ...
    for message in reqd_sticker_set.documents:
        if "image" in message.mime_type.split("/"):
            ...
            await eod(godfatherevent, "`Unsupported File!`")
            return
        if photo:
            splat = ("".join(args.text.split(maxsplit=1)[1:])).split()
            emoji = emoji or "⚜"
            ...
            try:
                godfather = Get(godfather)
                await args.client(godfather)
            except BaseException:
                pass
            packnick = pack_nick(username, pack, is_anim, is_vid)
            packname = pack_name(userid, pack, is_anim, is_vid)
            ...
            if (
                ...
                async with args.client.conversation("@Stickers") as conv:
                    pack, GodFathername = await newpacksticker(
                        godfatherevent,
                        conv,
                        cmd,
                        args,
                ...
                async with args.client.conversation("@Stickers") as conv:
                    pack, GodFathername = await add_to_pack(
                        godfatherevent,
                        conv,
                        args,
                        packname,
            ...
            if GodFathername is None:
                return
            if GodFathername not in blablapacks:
                blablapacks.append(GodFathername)
    ...
    await godfatherevent.edit(result)
...
async def lol(args):
    ...
    if message and message.media:
        ...
        return
    cmd = "/newvideo"
    packname = f"godfatherOp_{userid}_temp_pack"
    response = urllib.request.urlopen(
        urllib.request.Request(f"http://t.me/addstickers/{packname}")
    )
    htmlstr = response.read().decode("utf8").split("\n")
    ...
    await godfatherevent.edit("`Hold on, making sticker...`")
    async with args.client.conversation("@Stickers") as conv:
        otherpack, packname, emoji = await newpacksticker(
            godfatherevent,
            conv,
...
@godfather.godfather_cmd(
    ...
    info={
        ...
        "examples": [
            "{tr}gridpack -e👌 godfatherUserBot",
        ],
    },
)
async def pic2packcmd(event):
    ...
    godfatherevent = await eor(event, "__🔪Cropping and adjusting the image...__")
    try:
        ...
        emoji = "🤖"
    chat = "@Stickers"
    name = "GodFather_XD" + "".join(
        random.choice(list(string.ascii_lowercase + string.ascii_uppercase))
        for _ in range(16)
    )
    image = await _godfathertools.media_to_pic(godfatherevent, reply, noedits=True)
    if image[1] is None:
        return await eod(
    ...
    newimg.save(new_img)
    new_img.seek(0)
    godfatherevent = await event.edit("__Making the pack.__")
    async with event.client.conversation(chat) as conv:
        ...
        for im in images:
            ...
            i += 1
            await godfatherevent.edit(f"__Making the pack.\nProgress: {i}/{len(images)}__")
...
async def get_pack_info(event):
    ...
    if not isinstance(stickerset_attr, DocumentAttributeSticker):
        return await godfatherevent.edit("`This is not a sticker. Reply to a sticker.`")
    get_stickerset = await event.client(
        GetStickerSetRequest(
            InputStickerSetID(
                id=stickerset_attr.stickerset.id,
    ...
    await godfatherevent.edit(OUTPUT)
...
async def cb_sticker(event):
    ...
    godfatherevent = await eor(event, "`Searching sticker packs....`")
    scraper = cloudscraper.create_scraper()
    text = scraper.get(combot_stickers_url + split).text
    soup = bs(text, "lxml")
    results = soup.find_all("div", {"class": "sticker-pack__header"})
    if not results:
        return await eod(godfatherevent, "`No results found :(.`", 5)
    ...
    for pack in results:
        if pack.button:
            ...
            reply += f"\n **• ID: **`{packid}`\n [{packtitle}]({packlink})"
    await godfatherevent.edit(reply)
