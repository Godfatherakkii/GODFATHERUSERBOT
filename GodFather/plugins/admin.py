...
async def promote(event):
    ...
    if not user:
        return
    godfatherevent = await eor(event, "`Promoting...`")
    try:
...
async def demote(event):
    ...
    if not user:
        return
    godfatherevent = await eor(event, "`Demoting...`")
    newrights = ChatAdminRights(
        add_admins=None,
        invite_users=None,
        change_info=None,
...
async def _ban_person(event):
    ...
    godfatherevent = await eor(event, "`Whacking the pest!`")
    try:
...
async def nothanos(event):
    ...
    if not user:
        return
    godfatherevent = await eor(event, "`Unbanning...`")
    try:
        ...
        await godfatherevent.edit("`Uh oh my unban logic broke!`")
    except Exception as e:
...
async def startmute(event):
    ...
    if event.is_private:
        ...
        if event.chat_id == godfather.uid:
...
async def kick(event):
    ...
    if not user:
        return
    godfatherevent = await eor(event, "`Kicking...`")
    try:
        await event.client.kick_participant(event.chat_id, user.id)
    except Exception as e:
        return await godfatherevent.edit(f"{NO_PERM}\n{e}")
    if reason:
        await event.client.send_file(
            event.chat_id,
            bn_pic,
...
async def _iundlt(event):  # sourcery no-metrics
    ...
    godfatherevent = await eor(event, "`Searching recent actions .....`")
    ...
    if not type:
        for msg in adminlog:
            ...
            await eor(godfatherevent, deleted_msg)
    else:
        main_msg = await eor(godfatherevent, deleted_msg)
        for msg in adminlog:
            sweet = await event.client.get_entity(msg.old.from_id)
            _media_type = media_type(msg.old)
            if _media_type is None:
