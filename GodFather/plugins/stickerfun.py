...
def file_checker(template):
    if not os.path.isdir("./temp"):
        os.mkdir("./temp")
    tempname = "./temp/godfather_temp.png"
    fontname = "./temp/ArialUnicodeMS.ttf"
    urllib.request.urlretrieve(template, tempname)
    if not os.path.exists(fontname):
        urllib.request.urlretrieve(
            "https://github.com/ITS-godfatherBOT/RESOURCES/blob/master/Resources/Spotify/ArialUnicodeMS.ttf?raw=true",
            fontname,
        )
...
async def sticklet(event):
    ...
    # delete the godfatherbot command,
    ...
    fontsize = 230
    FONT_FILE = await get_font_file(event.client, "@godfather_Fonts", font_file_name)
    ...
    image_stream.name = "godfatherUserBot.webp"
...
@godfather.godfather_cmd(
    ...
    info={
        ...
        "examples": "{tr}twt godfatheruserbot",
    },
)
async def twt(event):
...
@godfather.godfather_cmd(
    ...
    info={
        ...
        "examples": "{tr}googl godfatheruserbot",
    },
)
async def twt(event):
