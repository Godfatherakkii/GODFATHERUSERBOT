...
from . import _godfathertools, convert_toimage, godfather, soft_deEmojify
...
async def ocr(event):
    ...
    godfatherevent = await eor(event, "`Reading...`")
    ...
    try:
        ...
        await godfatherevent.edit("`Couldn't read it.. you sure this readable !?`")
    test_file = await ocr_space_file(filename=output_file, language=lang_code)
    try:
        ParsedText = test_file["ParsedResults"][0]["ParsedText"]
    except BaseException:
        await godfatherevent.edit("`Couldn't read it.`\n`I guess I need new glasses.`")
    else:
        if cmd == "":
