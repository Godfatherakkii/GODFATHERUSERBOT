# speech to text module for godfatherUserBot by uniborg (@spechide)
import os
...
async def _(event):
    ...
    godfatherevent = await eor(event, "`Downloading to my local, for analysis  🙇`")
    required_file_name = await event.client.download_media(reply, Config.TEMP_DIR)
    await godfatherevent.edit("`Starting analysis, using IBM WatSon Speech To Text`")
    headers = {
    ...
    r = response.json()
    if "results" not in r:
        return await godfatherevent.edit(r["error"])
    ...
    await godfatherevent.edit(string_to_show)
