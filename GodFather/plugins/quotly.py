...
from ..helpers.utils import _godfathertools, get_user_from_event, reply_id
...
@godfather.godfather_cmd(
    ...
    info={
        ...
        "examples": ["{tr}qpic godfatherUserBot.", "{tr}qpic -b godfatherUserBot."],
    },
)
async def q_pic(event):  # sourcery no-metrics
    ...
    godfatherevent = await eor(event, "__Making Quote pic....__")
    ...
    output = io.BytesIO()
    if sticker:
        output.name = "godfatherUserBot.Webp"
        img.save(output, "webp")
    else:
        output.name = "godfatherUserBot.png"
        img.save(output, "PNG")
    output.seek(0)
...
@godfather.godfather_cmd(
    ...
    info={
        ...
        "examples": ["{tr}fq @godfatherBoy_XD hello bad boys and girls"],
    },
)
async def stickerchat(owoquotes):
    ...
    godfatherevent = await eor(owoquotes, "`Making quote...`")
    if cmd in ["rq", "q"]:
        try:
            user = (
...
async def _(event):
    ...
    chat = "@QuotLyBot"
    godfatherevent = await eor(event, "```Making a Quote```")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            if messages_id != []:
                ...
                return await eod(
                    godfatherevent, "`I guess you have used a invalid syntax`"
                )
