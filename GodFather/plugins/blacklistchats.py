...
@godfather.godfather_cmd(
    ...
    info={
        ...
        "description": "If you turn this on, then your GodFather won't work on the chats stored\
         in database by addblkchat cmd. If you turn it off even though you added chats to database\
         GodFather won't stop working in that chat.",
...
async def chat_blacklist(event):
    ...
    if input_str == "on":
        ...
        text = "__From now ChatBlackList ON, your GodFather doesn't work in that chats which u add in BlackList __.Stored in database."
        if len(blkchats) != 0:
            text += (
    ...
    if gvarstatus("blacklist_chats") is not None:
        ...
        text = "__Your GodFather is as free as a bird.It works in Every Chat .__"
        if len(blkchats) != 0:
            text += (
                "**Bot is reloading to apply the changes. Please wait for a minute**"
            )
