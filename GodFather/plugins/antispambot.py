...
# baning spmmers plugin for godfatherUserBot by @godfather_K_BOY
...
from . import BOTLOG, BOTLOG_CHATID, eor, godfather, logging, spamwatch

LOGS = logging.getLogger(__name__)
menu_category = "admin"
if Config.ANTISPAMBOT_BAN:

    @godfather.on(ChatAction())
    async def anti_spambot(event):  # sourcery no-metrics
        if not event.user_joined and not event.user_added:
            return
        user = await event.get_user()
        godfatheradmin = await is_admin(event.client, event.chat_id, event.client.uid)
        if not godfatheradmin:
            return
        godfatherbanned = None
        adder = None
        ignore = None
        if event.user_added:
            try:
        ...
        if is_gbanned(user.id):
            godfathergban = gbanned(user.id)
            if godfathergban.reason:
                hmm = await event.reply(
                    f"[{user.first_name}](tg://user?id={user.id}) was gbanned by you for the reason `{godfathergban.reason}`"
                )
            else:
                hmm = await event.reply(
            ...
            try:
                ...
                godfatherbanned = True
            except Exception as e:
                LOGS.info(e)
        if spamwatch and not godfatherbanned:
...
async def caschecker(event):
    ...
    try:
        ...
        await godfatherevent.edit("`CAS check failed: Admin privileges are required`")
        return
    except BaseException:
        await godfatherevent.edit("`CAS check failed`")
        return
    await godfatherevent.edit(text)
...
async def spamchecker(event):
    ...
    try:
        ...
        await godfatherevent.edit("`spamwatch check failed`")
        return
    await godfatherevent.edit(text)
