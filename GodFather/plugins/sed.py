...
async def group_has_sedbot(group):
    if isinstance(group, types.InputPeerChannel):
        full = await godfather(functions.channels.GetFullChannelRequest(group))
    elif isinstance(group, types.InputPeerChat):
        full = await godfather(functions.messages.GetFullChatRequest(group.chat_id))
    else:
        return False
...
@godfather.on(events.NewMessage)
async def on_message(event):
    last_msgs[event.chat_id].appendleft(event.message)


@godfather.on(events.MessageEdited)
