...
from ..helpers.utils import _format, _godfatherutils
...
@godfather.godfather_cmd(
    ...
    info={
        ...
        "usage": "{tr}ls <path>",
        "examples": "{tr}ls GodFather",
    },
)
async def ls(event):  # sourcery no-metrics
    "To list all files and folders."
    godfather = "".join(event.text.split(maxsplit=1)[1:])
    path = godfather or os.getcwd()
    if not os.path.exists(path):
        await eor(
            event,
            f"there is no such directory or file with the name `{godfather}` check again",
        )
        return
    path = Path(godfather) if godfather else os.getcwd()
...
async def rem(event):
    ...
    godfather = event.pattern_match.group(1)
    if godfather:
        path = Path(godfather)
    else:
    ...
    if not os.path.exists(path):
        ...
        return
    godfathercmd = f"rm -rf '{path}'"
    if os.path.isdir(path):
        await _godfatherutils.runcmd(godfathercmd)
...
async def mkdir(event):
    ...
    try:
        await _godfatherutils.runcmd(f"mkdir {original}")
...
async def cpto(event):
    ...
    await asyncio.sleep(2)
    try:
        await _godfatherutils.runcmd(f"cp -r {original} {location}")
