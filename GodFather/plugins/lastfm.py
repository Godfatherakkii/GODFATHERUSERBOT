...
async def get_curr_track(lfmbio):  # sourcery no-metrics
    ...
    while LASTFM_.LASTFMCHECK:
        try:
            ...
            user_info = (await godfather(GetFullUserRequest(LASTFM_.USER_ID))).full_user
            ...
            if (
                ...
                try:
                    ...
                    await godfather(UpdateProfileRequest(about=lfmbio))
                except AboutTooLongError:
                    short_bio = f"🎧: {LASTFM_.SONG}"
                    await godfather(UpdateProfileRequest(about=short_bio))
            if playing is None and user_info.about != DEFAULT_BIO:
                await sleep(6)
                await godfather(UpdateProfileRequest(about=DEFAULT_BIO))
                if BOTLOG and LASTFM_.LastLog:
                    await godfather.send_message(
            ...
            if BOTLOG and LASTFM_.LastLog:
                await godfather.send_message(BOTLOG_CHATID, f"Error changing bio:\n{err}")
        await sleep(2)
