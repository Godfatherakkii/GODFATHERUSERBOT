...
async def remove_background(event):
    ...
    if event.reply_to_msg_id and not input_str:
        ...
        godfatherevent = await eor(event, "`Analysing this Image/Sticker...`")
        file_name = os.path.join(Config.TEMP_DIR, "rmbg.png")
        try:
            await event.client.download_media(reply_message, file_name)
        except Exception as e:
            await eod(godfatherevent, f"`{e}`", 5)
            return
        else:
            await godfatherevent.edit("`Removing Background of this media`")
            file_name = convert_toimage(file_name)
            response = ReTrieveFile(file_name)
            os.remove(file_name)
    elif input_str:
        godfatherevent = await eor(event, "`Removing Background of this media`")
    ...
    if "image" in contentType:
        ...
        await eod(godfatherevent, f"`{response.content.decode('UTF-8')}`", 5)
        return
    if cmd == "srmbg":
