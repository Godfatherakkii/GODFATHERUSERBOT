...
async def truth_dare_task(event):
    ...
    if taskmode == "task":
        godfatherevent = await eor(event, "Getting a random task for you.....")
        taskmode = random.choice(["truth", "dare"])
    else:
        godfatherevent = await eor(event, f"Getting a random {taskmode} task for you.....")
    ...
    try:
        ...
        if taskmode == "truth":
            await godfatherevent.edit(f"**The truth task for you is**\n`{task}`")
        else:
            await godfatherevent.edit(f"**The dare task for you is**\n`{task}`")
    except Exception as e:
        await eod(godfatherevent, f"**Error while getting task**\n`{e}`", 7)
...
async def igame(event):
    ...
    if input_str not in game_code:
        godfatherevent = await eor(event, "`Give me a correct game code...`")
        await asyncio.sleep(1)
        await eod(
            godfatherevent, f"**Available Game Codes & Names :-**\n\n{game_list}", time=60
        )
    else:
        await eor(
            event,
