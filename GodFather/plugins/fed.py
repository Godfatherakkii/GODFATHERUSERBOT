...
from GodFather import BOTLOG, BOTLOG_CHATID, godfather
...
async def group_fban(event):
    ...
    if errors != []:
        ...
        for txt in errors:
            success_report += f"\n☞ __{txt}__"
    await eor(godfatherevent, success_report)
...
async def quote_search(event):  # sourcery no-metrics
    ...
    if get_collection("fedids") is not None:
        ...
        feds = {}
    if fedgroup == "-all":
        godfatherevent = await eor(event, "`Adding all your feds to database...`")
        fedidstoadd = []
        async with event.client.conversation("@MissRose_bot") as conv:
            try:
                try:
                    ...
                    await godfather(unblock("MissRose_bot"))
                    await conv.send_message("/myfeds")
                ...
                if "can only" in response.text:
                    return await eod(godfatherevent, f"__{response.text}__")
                ...
                await eod(godfatherevent, f"**Error while fecthing myfeds:**\n__{e}__", 10)
...
async def fetch_fedinfo(event):
    ...
    godfatherevent = await eor(event, "`Fetching info about given fed...`")
    async with event.client.conversation(rose) as conv:
        try:
            try:
                ...
                await conv.send_message(f"/fedinfo {input_str}")
            response = await conv.get_response()
            await eor(godfatherevent, response.text)
        except Exception as e:
            await eod(godfatherevent, f"**Error while fecthing fedinfo:**\n__{e}__", 10)
        await event.client.send_read_acknowledge(conv.chat_id)
        conv.cancel()
...
async def fetch_fedinfo(event):
    ...
    godfatherevent = await eor(event, "`Fetching admins list of given fed...`")
    async with event.client.conversation(rose) as conv:
        try:
            try:
...
async def myfeds_fedinfo(event):
    ...
    godfatherevent = await eor(event, "`Fetching list of feds...`")
...
async def fstat_rose(event):
    ...
    godfatherevent = await eor(event, "`Fetching fedstat from given deatils...`")
    user, fedid = await get_user_from_event(
        event, godfatherevent, secondgroup=True, noedits=True
    )
    if user is None:
        user = await event.client.get_me()
    if fedid is None:
    ...
    async with event.client.conversation(rose) as conv:
        try:
            ...
            if "Looks like" in response.message:
                ...
                await godfatherevent.delete()
                return
            await eor(godfatherevent, result + response.text)
        except YouBlockedUserError:
            await eod(
                godfatherevent,
            ...
            await eod(godfatherevent, f"**Error while fecthing fedstat:**\n__{e}__", 10)
        await event.client.send_read_acknowledge(conv.chat_id)
        conv.cancel()
