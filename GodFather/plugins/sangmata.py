...
async def _(event):  # sourcery no-metrics
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await eod(godfatherevent, "`unblock @Sangmatainfo_bot and then try`")
        responses = []
        while True:
            try:
    ...
    if not responses:
        await eod(godfatherevent, "`bot can't fetch results`")
    if "No records found" in responses:
        await eod(godfatherevent, "`The user doesn't have any record`")
    ...
    for i in check:
        if godfather:
            ...
            godfather = True
            await godfatherevent.edit(i, parse_mode=_format.parse_pre)
