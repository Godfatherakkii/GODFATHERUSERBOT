...
menu_category = "misc"
from . import godfather_channel

GIT_TEMP_DIR = "./temp/"

msg = f"""
**⚜ 𝙻𝚎𝚐𝚎𝚗𝚍𝚊𝚛𝚢 𝙰𝚏 GodFather ⚜**
  •        [♥️ 𝚁𝚎𝚙𝚘 ♥️](https://github.com/godfather-AI/GodFather)
  •        [♦️ Deploy ♦️](https://dashboard.heroku.com/new?button-url=https%3A%2F%2Fgithub.com%2Fgodfather-AI%2FGodFather&template=https%3A%2F%2Fgithub.com%2Fgodfather-AI%2FGodFather)
  •  ©️ {godfather_channel} ™
"""
...
@godfather.godfather_cmd(
    ...
    info={
        "header": "Source code link of GodFather",
        "usage": [
            "{tr}repo",
        ],
    },
...
async def source(e):
    "Source code link of GodFather"
    reply_to_id = await reply_id(e)
    try:
        godfather = await e.client.inline_query(Config.BOT_USERNAME, "repo")
        await godfather[0].click(e.chat_id, reply_to=reply_to_id, hide_via=True)
        await e.delete()
    except (noin, dedbot):
        await eor(e, msg)
...
@godfather.godfather_cmd(
    ...
    info={
        ...
        "examples": ["{tr}github godfather-AI", "{tr}github -l5 godfather-AI"],
    },
)
async def _(event):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(URL) as request:
            ...
            godfatherevent = await eor(event, "`fetching github info ...`")
...
@godfather.godfather_cmd(
    ...
    info={
        ...
        "description": "It uploads the given file to your github repo in **GodFather/plugins** folder\
...
async def download(event):
    ...
    if Config.GIT_REPO_NAME is None:
        return await eod(
            event, "`Please ADD Proper Github Repo Name of your GodFather`", 5
        )
...
async def git_commit(file_name, mone):
    ...
    if create_file:
        file_name = f"GodFather/plugins/{file_name}"
        LOGS.info(file_name)
        try:
            repo.create_file(
            ...
            ccess = ccess.strip()
            await mone.edit(
                f"`Commited On Your Github Repo`\n\n[Your PLUGINS](https://github.com/{ccess}/tree/master/GodFather/plugins/)"
            )
