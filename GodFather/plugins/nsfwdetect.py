...
async def detect(event):
    ...
    godfatherevent = await eor(event, "`Downloading the file to check...`")
    media = await event.client.download_media(reply)
    if not media.endswith(("png", "jpg", "webp")):
        return await eod(event, "`Reply to any image or non animated sticker !`", 5)
    godfatherevent = await eor(event, "`Detecting NSFW limit...`")
    r = requests.post(
    ...
    os.remove(media)
    if "status" in r.json():
        return await eod(godfatherevent, r.json()["status"])
