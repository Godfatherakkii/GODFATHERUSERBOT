...
async def tmuter(event):  # sourcery no-metrics
    ...
    godfatherevent = await eor(event, "`muting....`")
    user, reason = await get_user_from_event(event, godfatherevent)
    if not user:
        return
    if not reason:
        return await godfatherevent.edit("you haven't mentioned time, check `.help tmute`")
    reason = reason.split(" ", 1)
    hmm = len(reason)
    swttime = reason[0].strip()
    reason = "".join(reason[1:]) if hmm > 1 else None
    ctime = await extract_time(godfatherevent, swttime)
    if not ctime:
        return
    if user.id == event.client.uid:
        return await godfatherevent.edit("Sorry, I can't mute myself")
    try:
        await godfatherevent.client(
            EditBannedRequest(
                event.chat_id,
                user.id,
        ...
        return await godfatherevent.edit("`Uh oh my mute logic broke!`")
        ...
        return await godfatherevent.edit(f"`{e}`")
...
async def tban(event):  # sourcery no-metrics
    ...
    godfatherevent = await eor(event, "`banning....`")
    user, reason = await get_user_from_event(event, godfatherevent)
    if not user:
        return
    if not reason:
        return await godfatherevent.edit("you haven't mentioned time, check `.help tban`")
    reason = reason.split(" ", 1)
    hmm = len(reason)
    swttime = reason[0].strip()
    reason = "".join(reason[1:]) if hmm > 1 else None
    ...
    if not ctime:
        return
    if user.id == event.client.uid:
        return await godfatherevent.edit("Sorry, I can't ban myself")
    await godfatherevent.edit("`Whacking the pest!`")
    try:
        await event.client(
            EditBannedRequest(
                event.chat_id,
        ...
        return await godfatherevent.edit(NO_PERM)
