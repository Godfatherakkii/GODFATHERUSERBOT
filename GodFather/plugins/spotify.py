...
from GodFather.core.logger import logging
...
from . import BOTLOG, BOTLOG_CHATID, Config, godfather, reply_id
...
PATH = "GodFather/cache/spotify_database.json"
        ...
        while SP_DATABASE.SPOTIFY_MODE:
            ...
            if r.status_code == 200:
                ...
                if received["currently_playing_type"] == "track":
                    ...
                    if save_spam("spotify", False):
                        ...
                        await godfather.send_message(BOTLOG_CHATID, string)
                else:
            ...
            # need to change anything
                ...
                if save_spam("spotify", False):
                    ...
                    await godfather.send_message(BOTLOG_CHATID, stringy)
            ...
            try:
                ...
                full = (await godfather(GetFullUserRequest(godfather.uid))).full_user
                bio = full.about
                ...
                if to_insert:
                    ...
                    if new_bio:
                        ...
                        if not new_bio == bio:
                            try:
                                await godfather(UpdateProfileRequest(about=new_bio))
                    ...
                    if not new_bio:
                        if save_spam("telegram", True):
                            ...
                            await godfather.send_message(BOTLOG_CHATID, to_send)
                # not to_insert means no playback
                    ...
                    # back to the original one
                        ...
                        await godfather(UpdateProfileRequest(about=old_bio))
...
async def spotifybio(event):
    ...
    if SP_DATABASE.SPOTIFY_MODE:
        SP_DATABASE.SPOTIFY_MODE = False
        if USER_INITIAL_BIO:
            await godfather(UpdateProfileRequest(about=USER_INITIAL_BIO["bio"]))
        ...
        USER_INITIAL_BIO["bio"] = (
            (await godfather(GetFullUserRequest(godfather.uid))).full_user
        ).about or ""
...
def telegraph_lyrics(tittle, artist):
    ...
    try:
        response = telegraph.create_page(
            ...
            author_url="https://t.me/GodFather_XD",
        )
...
def file_check():
    ...
    if not os.path.exists(logo):
        urllib.request.urlretrieve(
            "https://github.com/ITS-GodFather/RESOURCES/raw/master/Resources/Spotify/godfather.png",
            logo,
        )
    if not os.path.exists(font_mid):
        urllib.request.urlretrieve(
            "https://github.com/ITS-GodFather/RESOURCES/blob/master/Resources/Spotify/ArialUnicodeMS.ttf?raw=true",
            font_mid,
        )
    if not os.path.exists(font_bold):
        urllib.request.urlretrieve(
            "https://github.com/ITS-GodFather/RESOURCES/blob/master/Resources/Spotify/GoogleSans-Bold.ttf?raw=true",
            font_bold,
        )
...
async def make_thumb(url, client, song, artist, now, full):
    ...
    try:
        ...
        myphoto = urllib.request.urlretrieve(
            "https://github.com/ITS-GodFather/RESOURCES/raw/master/Resources/Spotify/op.jpg"
        )
...
async def spotify_now(event):
    ...
    if link:
        ...
        if not url(link) and "spotify" not in link:
            return await eod(godfatherevent, "**Give me a correct link...**")
    elif not link:
        if not await sp_var_check(event):
            return
        r = sp_data("https://api.spotify.com/v1/me/player/currently-playing")
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await godfather(unblock("DeezerMusicBot"))
            purgeflag = await conv.send_message("/start")
        ...
        await godfatherevent.delete()
        if cmd == "i":
            songg = await godfather.send_file(BOTLOG_CHATID, song)
            fetch_songg = await godfather.tgbot.get_messages(BOTLOG_CHATID, ids=songg.id)
            btn_song = await godfather.tgbot.send_file(
                BOTLOG_CHATID, fetch_songg, buttons=Button.url("🎧 Spotify", link)
            )
            fetch_btn_song = await godfather.get_messages(BOTLOG_CHATID, ids=btn_song.id)
