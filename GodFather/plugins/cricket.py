...
plugin for @GodFather_OP
"""

from telethon.errors.rpcerrorlist import YouBlockedUserError

from . import eor, godfather, reply_id
...
async def _(event):
    ...
    godfatherevent = await eor(event, "```Gathering info...```")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await godfatherevent.edit("Unblock @cricbuzz_bot & try again")
            return
        if respond.text.startswith("I can't find that"):
            await godfatherevent.edit("sorry i can't find it")
        else:
