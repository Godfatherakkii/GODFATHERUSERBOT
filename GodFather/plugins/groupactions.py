...
async def ban_user(chat_id, i, rights):
    try:
        await godfather(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
...
async def _(event):
    ...
    godfatherevent = await eor(event, "__Unbanning all banned accounts in this group.__")
    succ = 0
    total = 0
    type = False
    ...
    async for i in event.client.iter_participants(
        ...
        try:
            ...
            await godfatherevent.edit(str(ex))
        else:
            succ += 1
            if type:
                await sleep(2)
...
async def rm_deletedacc(show):  # sourcery no-metrics
    ...
    if con != "clean":
        ...
        if type != " -r":
            ...
            godfatheradmin = await is_admin(show.client, show.chat_id, show.client.uid)
