# reverse search and google search  plugin for godfather
import io
import os
import re
import urllib
...
@godfather.godfather_cmd(
    ...
    info={
        ...
        "examples": [
            "{tr}gs godfatherUserBot",
            "{tr}gs -l6 godfatherUserBot",
            "{tr}gs -p2 godfatherUserBot",
            "{tr}gs -p2 -l7 godfatherUserBot",
        ],
    },
)
async def gsearch(q_event):
    "Google search command."
    godfatherevent = await eor(q_event, "`searching........`")
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        godfatherevent = await eor(event, "Pre Processing Media")
        ...
        if previous_message.media:
            ...
            the_location = google_rs_response.headers.get("Location")
        await godfatherevent.edit("Found Google Result. Pouring some soup on it!")
        headers = {
        ...
        try:
            ...
            return await eod(godfatherevent, "`Sorry. I am unable to find similar images`")
        end = datetime.now()
...
<i>fetched in {ms} seconds</i>""".format(
    ...
    await eor(godfatherevent, OUTPUT_STR, parse_mode="HTML", link_preview=False)
...
async def _(event):
    ...
    await godfatherevent.edit(f"`Downloaded to {downloaded_file_name}`")
    if downloaded_file_name.endswith((".webp")):
        resize_image(downloaded_file_name)
    try:
        media_urls = upload_file(downloaded_file_name)
    except exceptions.TelegraphException as exc:
        await godfatherevent.edit(f"**Error : **\n`{exc}`")
        os.remove(downloaded_file_name)
    else:
...
async def _(img):
    ...
    if message and message.media:
        ...
        return
    if photo:
        godfatherevent = await eor(img, "`Processing...`")
        try:
            image = Image.open(photo)
        except OSError:
            return await godfatherevent.edit("`Unsupported , most likely.`")
        name = "okgoogle.png"
        image.save(name, "PNG")
        image.close()
        ...
        if response != 400:
            ...
            return await godfatherevent.edit("`Unable to perform reverse search.`")
        ...
        if guess and imgspage:
            ...
            return await godfatherevent.edit("`Can't find any kind similar images.`")
