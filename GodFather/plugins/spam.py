...
async def spam_function(event, godfather, lol, sleeptimem, sleeptimet, DelaySpam=False):
    # sourcery no-metrics
    counter = int(lol[0])
    if len(lol) == 2:
        ...
        for _ in range(counter):
            ...
            if event.reply_to_msg_id:
                await godfather.reply(spam_message)
            else:
                await event.client.send_message(event.chat_id, spam_message)
            await asyncio.sleep(sleeptimet)
    elif event.reply_to_msg_id and godfather.media:
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            godfather = await event.client.send_file(
                event.chat_id, godfather, caption=godfather.text
            )
        ...
        if BOTLOG:
            ...
            godfather = await event.client.send_file(BOTLOG_CHATID, godfather)
            await _godfatherutils.unsavegif(event, godfather)
        return
    elif event.reply_to_msg_id and godfather.text:
        spam_message = godfather.text
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            await event.client.send_message(event.chat_id, spam_message)
...
async def spammer(event):
    ...
    godfather = await event.get_reply_message()
    ...
    await spam_function(event, godfather, lol, sleeptimem, sleeptimet)
...
@godfather.godfather_cmd(
    ...
    info={
        ...
        "usage": "{tr}cspam <text>",
        "examples": "{tr}cspam godfatheruserbot",
    },
)
...
@godfather.godfather_cmd(
    ...
    info={
        ...
        "usage": "{tr}wspam <text>",
        "examples": "{tr}wspam I am using godfatherUserBot",
    },
)
async def tmeme(event):
