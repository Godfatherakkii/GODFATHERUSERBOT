...
def plug_checker(plugin):
    plug_path = f"./GodFather/plugins/{plugin}.py"
    if not os.path.exists(plug_path):
        plug_path = f"./xtraplugins/{plugin}.py"
    if not os.path.exists(plug_path):
        plug_path = f"./GodFather/assistant/{plugin}.py"
    return plug_path
...
@godfather.godfather_cmd(
    ...
    info={
        ...
        "description": "Reply to any external plugin(supported by GodFather) to install it in your bot.",
        "usage": "{tr}install",
    },
)
async def install(event):
    ...
    b = 1
    owo = event.text[9:]
    godfather = await eor(event, "__Installing.__")
    if event.reply_to_msg_id:
        try:
            downloaded_file_name = (
                await event.client.download_media(  # pylint:disable=E0602
                    await event.get_reply_message(),
                    "./GodFather/plugins/",  # pylint:disable=E0602
                )
            )
            ...
            try:
                ...
                await eod(godfather, f"{e}")
                return os.remove(downloaded_file_name)
        except Exception as e:
            await eod(godfather, f"**Failed to Install** \n`Error`\n{str(e)}")
            return os.remove(downloaded_file_name)
