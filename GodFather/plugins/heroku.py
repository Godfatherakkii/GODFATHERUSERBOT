...
async def variable(var):  # sourcery no-metrics
    ...
    if exe == "get":
        godfather = await eor(var, "`Getting information...`")
        await asyncio.sleep(1.0)
        try:
            variable = var.pattern_match.group(2).split()[0]
            godfather = "**ConfigVars**:" f"\n\n {variable} = `{heroku_var[variable]}`\n"
            if "godfather_STRING" in variable:
                await eor(
                    var, "godfather String is a Sensetive Data.\nProtected By godfatherBot"
                )
                return
            elif variable in heroku_var:
                await eor(var, godfather)
            else:
        ...
        godfather = await eor(var, "`Setting information...`")
        if not variable:
            return await godfather.edit("`.set var <ConfigVars-name> <value>`")
        ...
        if "godfather_STRING" in variable:
            await godfather.edit("Successfully Changed")
            return
        ...
        godfather = await eor(var, "`Getting information to deleting variable...`")
        try:
            variable = var.pattern_match.group(2).split()[0]
        except IndexError:
            return await godfather.edit("`Please specify ConfigVars you want to delete`")
        await asyncio.sleep(1.5)
        if variable not in heroku_var:
            return await godfather.edit(f"`{variable}`**  does not exist**")

        await godfather.edit(f"`{variable}`  **successfully deleted**")
...
@godfather.godfather_cmd(
    ...
    info={
        "header": "To Check dyno usage of godfatherbot and also to know how much left.",
        "usage": "{tr}usage",
    },
)
