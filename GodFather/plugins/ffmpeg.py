...
FF_MPEG_DOWN_LOAD_MEDIA_PATH = os.path.join(
    Config.TMP_DOWNLOAD_DIRECTORY, "godfatherUserBot.media.ffmpeg"
)
...
async def ffmpeg_compress(event):
    ...
    if not reply_message or not reply_message.media:
        if os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
            ...
            godfatherevent = await eod(event, "`Processing...`")
            delete = False
        else:
        ...
        if media not in ["Video", "Round Video", "Gif"]:
            return await eod(event, "`Only Video files are supported`")
        godfatherevent = await eor(event, "`Saving the file...`")
        try:
            c_time = time.time()
            ...
            await event.client.fast_download_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, godfatherevent, c_time, "Trying to download")
                ),
            )
            dl.close()
            ...
            await eor(godfatherevent, "`Processing...`")
            delete = True
    else:
    ...
    compress = await convert_video(
        dlpath, "./temp", crf, old["duration"], godfather, godfatherevent
    )
    cend = datetime.now()
    ...
    if compress:
        ...
        if cmd == "f":
            try:
                ...
                catt = await event.client.send_file(
                    ...
                    progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                        progress(d, t, godfatherevent, c_time, "Trying to upload")
                    ),
                )
                os.remove(compress)
            except Exception as e:
                return await eod(godfatherevent, f"**Error : **`{e}`")
        else:
            thumb = await _godfathertools.take_screen_shot(compress, "00:01")
            try:
                c_time = time.time()
                catt = await event.client.send_file(
                    event.chat_id,
        ...
        return await eod(godfatherevent, "**ERROR :: Unalble to Compress**")
...
async def ff_mpeg_trim_cmd(event):
    ...
    reply_to_id = await reply_id(event)
    godfatherevent = await eor(event, "`Triming the media......`")
    ...
    if len(cmt) == 3:
        ...
        if o is None:
            return await eod(godfatherevent, "**Error : **`Can't complete the process`")
        try:
            c_time = time.time()
        ...
        cmd, start_time = cmt
        o = await _godfathertools.take_screen_shot(
        ...
        await eod(godfatherevent, "RTFM")
        return
    end = datetime.now()
    ms = (end - start).seconds
    await eod(godfatherevent, f"`Completed Process in {ms} seconds`", 3)
