...
async def lol(event):
    ...
    async with event.client.conversation("@distortionerbot") as conv:
        ...
        if out in ["Gif", "Video", "Sticker"]:
            await _godfatherutils.unsavegif(event, owo)
...
async def some(event):
    ...
    godfatherevent = await eor(event, "`Sending gif....`")
    ...
    for items in rlist:
        ...
        await _godfatherutils.unsavegif(event, nood)
...
async def some(event):
    ...
    godfatherevent = await eor(event, "`Wait babe...`😘")
