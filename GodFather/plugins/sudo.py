...
@godfather.godfather_cmd(
    ...
    info={
        "header": "To enable or disable sudo of your godfatheruserbot.",
...
async def chat_blacklist(event):
    "To enable or disable sudo of your godfatherUserBot."
...
async def _(event):
    ...
    try:
        ...
        sudousers = {}
    if len(sudochats) == 0:
        return await eod(event, "__There are no sudo users for your godfatheruserbot.__")
    result = "**The list of sudo users for your godfatherUserbot are :**\n\n"
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str[0] == "-all":
        godfatherevent = await eor(event, "__Enabling all safe cmds for sudo....__")
        totalcmds = CMD_INFO.keys()
        typecmds = (
            PLG_INFO["botcontrols"]
            + PLG_INFO["autoprofile"]
        ...
        godfatherevent = await eor(event, "__Enabling compelete sudo for users....__")
        loadcmds = CMD_INFO.keys()
        if len(sudocmds) > 0:
            sqllist.del_keyword_list("sudo_enabled_cmds")
    elif input_str[0] == "-p":
        godfatherevent = event
        input_str.remove("-p")
        loadcmds = []
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += (
                    f"`{plugin}` __There is no such plugin in your godfatherUserBot__.\n"
                )
            else:
        ...
        for cmd in input_str:
            if cmd not in CMD_INFO:
                errors += (
                    f"`{cmd}` __There is no such command in your godfatherUserBot__.\n"
                )
    ...
    for cmd in loadcmds:
        sqllist.add_to_list("sudo_enabled_cmds", cmd)
    result = (
        f"__Successfully enabled __ `{len(loadcmds)}` __ for godfatherUserBot sudo.__\n"
    )
    output = (
    ...
    if errors != "":
        output += "\n**Errors:**\n" + errors
    msg = await eor(godfatherevent, output)
    await event.client.reload(msg)
...
async def _(event):  # sourcery no-metrics
    ...
    input_str = input_str.split()
    if input_str[0] == "-all":
        godfatherevent = await eor(event, "__Disabling all enabled cmds for sudo....__")
        typecmds = sudocmds
    elif input_str[0] == "-type":
        godfatherevent = await eor(event, "__Disabling all typeged cmds for sudo.....__")
        typecmds = (
            PLG_INFO["botcontrols"]
            + PLG_INFO["autoprofile"]
            + PLG_INFO["evaluators"]
    ...
    result = f"__Successfully disabled __ `{count}` __ for godfatherUserBot sudo.__\n"
    output = (
        result + "**Bot is reloading to apply the changes. Please wait for a minute**\n"
    )
    if errors != "":
...
godfather.loop.create_task(_init())
