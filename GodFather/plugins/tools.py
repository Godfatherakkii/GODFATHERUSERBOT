...
async def currency(event):
    ...
    try:
        ...
        symbols = await AioHttp().get_raw(
            "https://raw.githubusercontent.com/ITS-godfatherBOT/RESOURCES/master/Resources/Data/currency.py"
        )
...
async def scan(event):
    ...
    chat = "@VS_Robot"
    godfatherevent = await eor(event, " `Sliding my tip, of fingers over it`")
    async with event.client.conversation(chat) as conv:
        try:
            flag = await conv.send_message("/start")
        except YouBlockedUserError:
            await eor(
                godfatherevent, "**Error:** Trying to unblock & retry, wait a sec..."
            )
            await godfather(unblock("VS_Robot"))
            flag = await conv.send_message("/start")
        ...
        if response1.text:
            ...
            await eod(godfatherevent, f"**Please wait for {sec}s before retry**", 15)
        else:
            await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            ...
            await event.client.send_read_acknowledge(conv.chat_id)
            if not input_str:
                await eor(godfatherevent, response3.text[30:])
            else:
...
async def parseqr(event):
    ...
    godfatherevent = await eor(event, "`Decoding....`")
    ...
    t_response, e_response = (await _godfatherutils.runcmd(command_to_exec))[:2]
    if os.path.exists(downloaded_file_name):
        os.remove(downloaded_file_name)
    soup = BeautifulSoup(t_response, "html.parser")
    try:
        qr_contents = soup.find_all("pre")[0].text
        await eor(godfatherevent, f"**The decoded message is :**\n`{qr_contents}`")
    except IndexError:
        result = soup.text
        await eor(godfatherevent, f"**Failed to Decode:**\n`{result}`")
    except Exception as e:
        await eor(godfatherevent, f"**Error:**\n`{e}`")
...
async def _(event):
    ...
    try:
        ...
        return await godfatherevent.edit(str(e))
    end = datetime.now()
    ms = (end - start).seconds
    await eod(godfatherevent, "Created BarCode in {} seconds".format(ms))
...
async def _(event):
    ...
    godfatherevent = await eor(event, "`processiong...........`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
    ...
    if not r.ok:
        return await godfatherevent.edit("xkcd n.{} not found!".format(xkcd_id))
    data = r.json()
    year = data.get("year")
...
Year: {}""".format(
    ...
    await godfatherevent.edit(output_str, link_preview=True)
