...
from . import BOTLOG, BOTLOG_CHATID, godfather
...
async def download_audio(event):
    ...
    godfatherevent = await eor(event, "`Preparing to download...`")
    ...
    for url in urls:
        try:
            ...
            vid_data = {"title": url, "uploader": "godfatheruserbot", "formats": []}
        startTime = time()
        ...
        for _path in glob.glob(os.path.join(Config.TEMP_DIR, str(startTime), "*")):
            if _path.lower().endswith((".jpg", ".png", ".webp")):
                ...
                _fpath = _path
        if not _fpath:
            return await eod(godfatherevent, "__Unable to upload file__")
...
async def download_video(event):
    ...
    reply_to_id = await reply_id(event)
    for url in urls:
        ytdl_data = await ytdl_down(godfatherevent, video_opts, url)
        if ytdl_down is None:
            return
        try:
            f = pathlib.Path("godfather_ytv.mp4")
            print(f)
            godfatherthumb = pathlib.Path("cat_ytv.jpg")
            if not os.path.exists(godfatherthumb):
                godfatherthumb = pathlib.Path("cat_ytv.webp")
            if not os.path.exists(godfatherthumb):
                godfatherthumb = None
            await godfatherevent.edit(
                f"`Preparing to upload video:`\
...
async def insta_dl(event):
    ...
    v2 = "@videomaniacbot"
    media_list = []
    godfatherevent = await eor(event, "**Downloading.....**")
    async with event.client.conversation(v1) as conv:
        try:
            ...
            await godfather(unblock("instasave_bot"))
            v1_flag = await conv.send_message("/start")
        ...
        await eor(godfatherevent, "**Switching v2...**")
        async with event.client.conversation(v2) as conv:
            try:
                ...
                await godfather(unblock("videomaniacbot"))
                v2_flag = await conv.send_message("/start")
