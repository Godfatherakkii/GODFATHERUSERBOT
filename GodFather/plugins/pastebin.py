...
async def paste_img(event):
    ...
    d_file_name = None
    godfatherevent = await eor(event, "`Pasting the text on image`")
...
async def paste_bin(event):
    ...
    godfatherevent = await eor(event, "`pasting text to paste bin....`")
    ...
    try:
        ...
        await godfatherevent.edit(result, link_preview=False, parse_mode="html")
    except Exception as e:
        await eod(godfatherevent, f"**Error while pasting text:**\n`{e}`")
...
async def get_dogbin_content(event):
    ...
    if not url and textx.text:
        ...
        for iurl in urls:
            if (
                ...
                or ("spaceb" in iurl)
                or ("nekobin" in iurl)
                or ("godfatherbin" in iurl)
            ):
                url = iurl
                break
    if not url:
        return await eod(event, "__I can't find any pastebin link.__")
    godfatherevent = await eor(event, "`Getting Contents of pastebin.....`")
    ...
    if rawurl is None:
        ...
        elif "godfatherbin" in url:
            rawurl = f"http://godfatherbin.up.railway.app/raw/{fid[0]}"
        else:
    ...
    try:
        ...
        return await godfatherevent.edit(f"**Request timed out.**__{str(TimeoutErr)}__")
    ...
    await eor(godfatherevent, reply_text)
...
async def _(event):
    ...
    try:
        ...
        return await eod(godfatherevent, f"**Error while pasting text:**\n`{e}`")
    url = response["url"]
    chat = "@CorsaBot"
    await godfatherevent.edit("`Making instant view...`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            return await godfatherevent.edit("```Please unblock me (@CorsaBot) and try```")
        result = ""
        if response:
        ...
        await godfatherevent.edit(result, link_preview=True)
