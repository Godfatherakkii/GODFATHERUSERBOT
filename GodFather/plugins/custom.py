    ...
    if vname in vlist:
        ...
        if cmd == "set":
            if vname == "DEFAULT_USER":
                ...
                USERINFO = await godfather.get_entity(legebd.uid)
                FULL_USERINFO = (await godfather(GetFullUserRequest(godfather.uid))).full_user
                ...
                if FULL_USERINFO.about:
                    ...
                    delgvar("DEFAULT_BIO")
                try:
                    photos = await godfather.get_profile_photos(godfather.uid)
                    myphoto = await godfather.download_media(photos[0])
                ...
                if not vinfo and vname == "ALIVE_TEMPLATE":
                    return await eod(event, "Check @GodFather_Alive")
                if not vinfo:
                    return await eod(
                        event,
...
@godfather.godfather_cmd(
    ...
    info={
        "header": "To customize your godfatherUserBot.",
        "options": {
...
async def custom_godfatherUserBot(event):
    "To customize your godfatherUserBot."
    reply = await event.get_reply_message()
    text = None
    if reply:
        text = reply.text
