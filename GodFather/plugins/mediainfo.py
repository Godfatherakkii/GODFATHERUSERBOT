...
# ported to telethon by @godfather_K_BOY (@godfather_K_BOY)
import os
...
async def mediainfo(event):
    ...
    if not reply.media:
        await eor(event, "reply to media to get info")
        return
    godfatherevent = await eor(event, "`Gathering ...`")
    X_MEDIA = reply.file.mime_type
    if (not X_MEDIA) or (X_MEDIA.startswith(("text"))):
        return await godfatherevent.edit("Reply To a supported Media Format")
    hmm = await file_data(reply)
    file_path = await reply.download_media(Config.TEMP_DIR)
    out, err, ret, pid = await _godfatherutils.runcmd(f"mediainfo '{file_path}'")
    if not out:
        out = "Not Supported"
    body_text = f"""
<h2>JSON</h2>
