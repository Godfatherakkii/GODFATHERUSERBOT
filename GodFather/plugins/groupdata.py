...
@godfather.godfather_cmd(
    ...
    info={
        ...
        "examples": "{tr}admins @godfatherBot_OP",
    },
)
async def _(event):
...
@godfather.godfather_cmd(
    ...
    info={
        ...
        "examples": "{tr}bots @godfatherBot_AI",
    },
)
async def _(event):
    "To get list of bots."
...
async def get_users(show):
    ...
    godfatherevent = await eor(show, "`getting users list wait...`  ")
    try:
        ...
        mentions += f" {str(err)} \n"
        try:
            await eor(godfatherevent, mentions)
...
@godfather.godfather_cmd(
    ...
    info={
        ...
        "examples": "{tr}chatinfo @godfatherBot_AI",
    },
)
async def info(event):
    "To get group information"
    godfatherevent = await eor(event, "`Analysing the chat...`")
    chat = await get_chatinfo(event, godfatherevent)
    if chat is None:
        return
    caption = await fetch_info(chat, event)
    try:
        await godfatherevent.edit(caption, parse_mode="html")
    except Exception as e:
        if BOTLOG:
        ...
        await godfatherevent.edit("`An unexpected error has occurred.`")


async def get_chatinfo(event, godfatherevent):
    ...
    try:
        ...
        try:
            ...
            await godfatherevent.edit("`Invalid channel/group`")
            return None
            ...
            await godfatherevent.edit("`Channel or supergroup doesn't exist`")
            return None
        except (TypeError, ValueError) as err:
            LOGS.info(err)
            await eod(godfatherevent, "**Error:**\n__Can't fetch the chat__")
            return None
    return chat_info
