...
async def cpu(event):
    ...
    await eor(
        event, f"**[godfather's](tg://need_update_for_some_feature/) CPU Model:**\n{o}"
    )
...
async def sysdetails(sysd):
    ...
    godfatherevent = await eor(sysd, "`Fetching system information.`")
    ...
    a, b, c, d = await _godfatherutils.runcmd(neo)
    result = str(a) + str(b)
    await eor(godfatherevent, "**Neofetch Result:** `" + result + "`")
