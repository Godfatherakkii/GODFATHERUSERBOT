...
godfatherboy = 2024465080


from telethon import Button, custom, events

from .core.logger import logging
from .core.session import godfather, tgbot

LOGS = logging.getLogger("godfatherUserBot")
godfather_PIC = "https://te.legra.ph/file/d3785d49b2c5e6f00bc67.jpg"
...
async def killer():
    godfather_USER = bot.me.first_name
    The_godfatherBoy = bot.uid
    legd_mention = f"[{godfather_USER}](tg://user?id={The_godfatherBoy})"
    name = f"{legd_mention}'s Assistant"
    description = f"I am Assistant Of {legd_mention}.This Bot Can Help U To Chat With My Master  @GodFather_XD"
    starkbot = await godfather.tgbot.get_me()
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            ...
            await asyncio.sleep(1)
            await bot.send_file(
                "@BotFather", "GodFather/helpers/resources/pics/main.jpg"
            )
...
@godfather.tgbot.on(events.NewMessage(pattern="/start", func=lambda x: x.is_group))
...
@godfather.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"start")))
async def help(event):
    ...
    if event.query.user_id is not bot.uid:
        ...
        await tgbot.send_message(
            event.chat_id,
            message=f"Hey, I am your {bot_id}'s assistant bot.\nI Am Here To Help U \n\nPowered By [GodFather](https://t.me/GodFather_OP)",
            buttons=[
                [
                    Button.url(" Support ", "https://t.me/GodFather_OP"),
                    Button.url(" Updates ", "https://t.me/GodFather_AI"),
                ],
                [
...
@godfather.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"res_tart")))
async def res_ttart(event):
    if event.query.user_id == bot.uid:
        await event.answer("Restarting Please Wait 4 min... ", cache_time=0, alert=True)
        if BOTLOG:
            godfather = await event.client.send_message(
                BOTLOG_CHATID, "# RESTART \n" "Bot Restarted"
            )
        try:
            await godfather.disconnect()
...
@godfather.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"setvar")))
...
@godfather.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"Ahack")))
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        if len(i) > 3855:
            ...
            file.write(i + "\n\nDetails BY godfatherBoy")
            file.close()
            await bot.send_file(event.chat_id, "session.txt")
            system("rm -rf session.txt")
        else:
            await event.reply(
                i + "\n\nThanks For using godfatherBoyBot. \n/hack", buttons=keyboard
            )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            i + "\n\nThanks For using godfatherBoy Bot.\n/hack", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Banning all members. Thanks For using godfatherBoy Bot", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(i + "\n\nThanks For using godfatherBoy Bot", buttons=keyboard)
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        if i:
            await event.reply(
                "User don't have two step thats why now two step is `godfatherBoy Bot Is best` you can login now\n\nThanks For using godfatherBoy Bot.",
                buttons=keyboard,
            )
        else:
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        try:
            ...
            await event.respond(
                "SEND THIS ERROR TO - @godfather_Userbot\n**LOGS**\n" + str(e)
            )
